...
turtles-own [
  ...
  turtleType         ;; 1 = Follower, 2 = informed turtle (S1), 3 = informed turtle (S2)
]

to setup
  clear-all
  reset-ticks
  let followingTurtles (population * (1 - ((S1 / 100) + (S2 / 100))))
  create-turtles followingTurtles
  ...
  let turtleS1 (population * (S1 / 100))
  let turtleS2 (population * (S2 / 100))
  create-turtles turtleS1
  [
        set color blue
        set size 1.5
        setxy random-xcor random-ycor
        set heading S1-Direction
        set flockmates no-turtles
        set turtleType 2
  ]
    create-turtles turtleS2
  [
        set color red
        set size 1.5
        setxy random-xcor random-ycor
        set heading S2-Direction
        set flockmates no-turtles
        set turtleType 3
  ]
  reset-ticks
...
to calcAvgFlockHeading
ask turtle 0
  [
    let x-component sum [dx] of turtles
    let y-component sum [dy] of turtles
    ifelse x-component = 0 and y-component = 0
    [ set avgHeading heading ]
    [ set avgHeading atan x-component y-component]
  ]
end
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot avgHeading"

@#$#@#$#@
## WHAT IS IT?

This model is an attempt to mimic the flocking of birds described by Couzin. There are two types of informed birds. S1 and S2. S1 and S2 parameters set the percentage of population of each kind of bird in the total population of birds. S1 has a predefined heading set by S1-Direction and S2 has a predefined setting set by S2-Direction. The study shows that a flock of birds will follow the driection of informed birds.   
